...
main_application.title('Matrix Text Editor ')
...
italilc_btn =ttk.Button(tool_bar,image=italic_icon)
italilc_btn.grid(row=0,column=3,padx=5)
...
text_editor.config(wrap='word',relief = tk.RAISED)
...
italilc_btn.configure(command=change_italic)

## underline button functionlity
...
def save_file(event=None):
    ...
    try:
        if url :
            ...
            conten2 = text_editor.get(1.0,tk.END)
            url.write(conten2)
            url.close()
    except:
        return
